  ...
  entries.forEach((entry) => {
    if (entry.isIntersecting && !storyTitleChanged) {
      setTimeout(() => {
        ...
        storyParagraphs[0].textContent = "Uma garota e um rapaz se conheceram e se perderam em horas de conversa. Para ele, ela era a pessoa mais deslumbrante que seus olhos já tinham visto. A conexão foi tão leve e agradável que o garoto sentiu que poderia passar a vida inteira ali, vivendo naquele único diálogo.";
...
const closeWelcome = () => {
  ...
  heroTitle.innerHTML = 'Quero fazer você <em>sentir especial todos os dias.</em><span>Como a personagem principal do meu Dorama</span>';
